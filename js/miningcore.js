...
function loadHomePage(){
	...
	.done(function (data) {
		...
		$.each(filteredData, function (index, value) {
			...
			var pool_mined = true;
			var pool_networkstat_hash = "&nbsp;Loading New Coin...&nbsp;";
			var pool_networkstat_diff = "&nbsp;Loading New Coin...&nbsp;";
			var pool_networkstat_blockheight = "&nbsp;Loading New Coin...&nbsp;";
			var pool_stat_miner = "&nbsp;Loading New Coin...&nbsp;";
			var pool_stat_hash = "&nbsp;Loading New Coin...&nbsp;";
			var pool_netWorkShare = "&nbsp;Loading New Coin...&nbsp;";
...
function loadConnectPage() {
  console.log("Starting:", stratumConnectionInfo)
  stratumConnectionInfo = []
    ...
    .done(function(data) {
	  ...
	  $.each(value.ports, function (port, options) {
		...
		stratumAddressPrefix.forEach(function(prefix) {
			connectPoolConfig += "<tr><td><img class=\"stratum stratum-id-" + id + "\" src=\"img/flags/" + prefix + ".svg\" width=\"20\" height=\"20\">&nbsp;" + prefix + ": stratum+tcp://" + prefix + "." + stratumAddress + ":" + port + " ping: <span id='ping-" + id + "'></span></td><td>";
			var poolConnection = {
				id: id, 
				url: prefix + "." + stratumAddress + ":" + port
			};
			id++;
			//ping(poolConnection.url, updateStratumDisplay)
			stratumConnectionInfo.push(poolConnection);
			console.log("Adding url to array",stratumConnectionInfo);
...
async function loadStatsData() 
{
    ...
    if (value.coin.family == "alephium") {
	  ...
	  { $("#poolEffort").html(PoolEffortHTML); }
	  else {$("#poolEffort").html("Not Applicable for Solo-mining");}
		...
		if (value.paymentProcessing.payoutScheme !== "SOLO")
			{ $("#text_TTFBlocks").html(readableSeconds(ttf_blocks)); }
		else {$("#text_TTFBlocks").html("Not Applicable for Solo-mining");}		
